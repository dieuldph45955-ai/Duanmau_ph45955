...
import com.example.duanmau_ph45955.adapter.GioHangAdapter;
...
import com.example.duanmau_ph45955.utils.AppContext;
import com.example.duanmau_ph45955.utils.GioHangItem;
...
public class GioHangActivity extends AppCompatActivity implements GioHangAdapter.OnSanPhamClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        gioHangAdapter = new GioHangAdapter(this, AppContext.getGioHang().getDanhSachGioHang());
        ...
        btnThanhToan.setOnClickListener(v -> {
            ...
            int tongTien = 0;
            for (GioHangItem item : AppContext.getGioHang().getDanhSachGioHang()) {
                tongTien += item.getSanPham().getGiaSanPham() * item.getSoLuong();
            }
            db.themHoaDon(new HoaDon(maHoaDon, AppContext.maNhanVien, maKhachHang, dateFormat.format(new Date()), tongTien));
            ...
            AppContext.getGioHang().getDanhSachGioHang().clear();
            finish();
        });
    }
    ...
    public void onDeleteSanPham(GioHangItem gioHangItem) {
        AppContext.getGioHang().getDanhSachGioHang().remove(gioHangItem);
