package com.example.duanmau_ph45955.database;
...
import com.example.duanmau_ph45955.dto.HoaDonChiTietDto;
import com.example.duanmau_ph45955.dto.HoaDonDto;
import com.example.duanmau_ph45955.model.DanhMuc;
import com.example.duanmau_ph45955.model.HoaDon;
import com.example.duanmau_ph45955.model.HoaDonChiTiet;
import com.example.duanmau_ph45955.model.KhachHang;
import com.example.duanmau_ph45955.model.NhanVien;
import com.example.duanmau_ph45955.model.SanPham;
import com.example.duanmau_ph45955.model.TopKhachHang;
...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    // Hàm hỗ trợ chuyển đổi tiếng Việt có dấu thành không dấu để tìm kiếm trên máy ảo
    public String removeAccent(String s) {
        ...
        return pattern.matcher(temp).replaceAll("")
                ...
                .replace(' ', ' '); // Giữ nguyên khoảng trắng
    }
    ...
    public List<SanPham> timKiemSanPham(String tenSP) {
        ...
        //1. Lấy tất cả sản phẩm ra để lọc bằng Java
        // Chú ý: Dòng dưới đây đã được sửa để không bị dính vào comment
        Cursor cursor = db.rawQuery("SELECT * FROM " + BANG_SANPHAM, null);

        // Chuẩn hóa từ khóa người dùng nhập (ví dụ máy ảo gõ: "nuoc" -> "nuoc")
        String keySearch = removeAccent(tenSP).toLowerCase().trim();

        if (cursor.moveToFirst()) {
            do {
                String tenTrongDB = cursor.getString(1); // Cột tên sản phẩm (ví dụ: "Nước Ngọt")

                // Chuẩn hóa tên trong CSDL (ví dụ: "Nước Ngọt" -> "nuoc ngot")
                String tenTrongDBKhongDau = removeAccent(tenTrongDB).toLowerCase();

                // 2. So sánh: Nếu tên không dấu chứa từ khóa không dấu thì mới thêm vào danh sách
                if (tenTrongDBKhongDau.contains(keySearch)) {
                    SanPham sp = new SanPham(
                            cursor.getString(0), // Mã sản phẩm
                            cursor.getString(1), // Tên sản phẩm gốc (giữ nguyên có dấu để hiển thị)
                            cursor.getInt(2),    // Giá
    ...
    public List<NhanVien> timKiemNhanVien(String tenNV) {
        ...
        // 1. Lấy tất cả bản ghi từ bảng NhanVien
        Cursor cursor = db.rawQuery("SELECT * FROM " + BANG_NHANVIEN, null);

        String key = removeAccent(tenNV).toLowerCase().trim();
